...
- **Edge Motion**: **Animate Selected Edges Only** toggle — when on, the edges along the selected nodes' path to the root animate in the chosen **Selected Edge Pattern** (dashed or dotted). The sidebar's **Motion** and **Pattern** controls then apply to all *other* (non-selected) edges. When off, sidebar controls apply globally. The all-edges motion toggle lives in the sidebar (Power User mode), not in this dialog.
