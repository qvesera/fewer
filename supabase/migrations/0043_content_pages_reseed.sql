-- Re-seed content_pages from the markdown in content/docs/ and content/blog/.
--
...
values ('blog', 'aurora-haze-theme', 'Aurora Haze: A New Visual Identity for Fewer', 'How we designed and implemented the Aurora Haze design system: a subtle warm atmospheric effects, motion tokens, and a refined sidebar that makes directory exploration feel alive.', 'Version 0.2.5 introduces **Aurora Haze**: a cohesive design language that transforms Fewer from a functional tool into a visually distinctive experience.
...
', 'Yash Srivastava', '2026-08-05', 'design, theme, ux, release', true)
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('blog', 'elk-layout-engine', 'Custom Layout Algorithm: Built for Large Codebases', 'How we replaced Dagre with a custom Reingold-Tilford tree layout to handle large graphs for tighter spacing, better hierarchy, async layout for smooth imports.', 'Version 0.2.3 ships a new layout engine: a **custom Reingold-Tilford tree layout** with contour matching, designed specifically for directory trees. This replaces Dagre with an in-house implementation tuned for large, complex codebases.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('blog', 'first-release', 'Introducing Fewer: Turn Any Directory Into an Interactive Graph', 'Meet Fewer, the open-source tool that transforms static directory trees into interactive, explorable graphs. Built with React Flow, a custom tree layout, and a privacy-first philosophy.', 'We''re excited to announce the first public release of **Fewer**, an interactive directory graph visualizer that runs entirely in your browser.
...
', 'Yash Srivastava', '2026-08-01', 'launch, release, features', true)
on conflict (type, slug) do update
  set title = excluded.title,
      description = excluded.description,
      content = excluded.content,
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('blog', 'v030-release', 'v0.3.0: Theme Engine, 18 Presets, and a Lighter Bundle', 'Fewer 0.3.0 brings a structured custom theme engine with 18 presets, a draggable minimizable theme editor, and a bundle that''s 300KB lighter after dead-code removal.', 'Today we ship **Fewer 0.3.0**: the biggest update since launch. This release focuses on three things: making the tool yours, making it faster to load, and making it feel like a real app.
...
', 'Yash Srivastava', '2026-08-06', 'release, themes, performance, pwa', true)
on conflict (type, slug) do update
  set title = excluded.title,
      description = excluded.description,
      content = excluded.content,
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('blog', 'v060-release', 'v0.6.0: A Gallery, Version History, and One Import Dialog', 'Fewer 0.6.0 adds a public community gallery, automatic version history you can restore, username-or-email sign-in with real profiles, and a single 3-step import dialog that handles folder, file, URL, and cloud sources alike.', '**v0.6.0** is the release where Fewer stopped being only a viewer of directories you already had, and became a place where directory graphs can be **published, versioned, and built from almost any source**.
...
bun run dev
```
', 'Yash Srivastava', '2026-09-12', 'release, gallery, sharing, import, accounts', true)
on conflict (type, slug) do update
  set title = excluded.title,
      description = excluded.description,
      content = excluded.content,
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'accounts', 'Accounts & Saved Graphs', 'Sign in to save your directories, access them across devices, and share them. Covers sign in, save, load, rename, delete, and theme/settings sync.', 'Fewer works fully without an account. Signing in is optional and unlocks saving your directories to your account, accessing them from any device, and sharing them with more control.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'cloud', 'Cloud Storage', 'Link cloud accounts (GitHub, Google Drive, OneDrive, SharePoint, Azure) to browse and visualize folders from cloud storage and private repositories.', 'Fewer can browse and visualize directory structures from linked cloud accounts — private GitHub repos, Google Drive, OneDrive, SharePoint, and Azure. Sign in, link an account, then browse and import a folder into the graph.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'deployment', 'Deployment & Self-Hosting', 'Deploy Fewer to production: Docker, Netlify, Caddy reverse proxy, and the standalone build.', 'Fewer is a Next.js 16 app that can be deployed anywhere Node.js runs. This guide covers the supported deployment paths.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'editing', 'Editing Cards', 'Add, rename, copy, cut, paste, duplicate, delete, and connect nodes. Explore context menu actions for folders and files, plus the clipboard and undo/redo.', 'Fewer treats your graph like an editable outline. Every node supports the full set of editing actions, either from the **context menu** (right-click) or via **keyboard shortcuts**.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'getting-started', 'Getting Started with Fewer', 'Install, import your first directory, and navigate the graph. Everything you need to start exploring directory structures visually.', 'There is a web version available at [https://app.fewer.directory/app](https://app.fewer.directory/app).
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'graph-features', 'Graph Features', 'Deep dive into Fewer''s graph visualization: React Flow canvas, custom node types, layout engines, connection styles, and navigation features.', 'Fewer is a very feature-rich directory viewer. Here is a deep dive into all of its features:
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'import-export', 'Import & Export', 'Import directories from disk, GitHub, or files. Export your graph as SVG, PNG, JSON, CSV, DOT, shell scripts, or ASCII trees.', 'Fewer lets you import file trees in multiple formats, whether it is directly from your disk, a github url, or from a previously exported file.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'privacy', 'Privacy Policy', 'How fewer collects, uses, and protects your data. Fewer runs in your browser with no trackers, no ads, and no telemetry. This policy explains exactly what we handle, why, and how you can control it.', '**Last updated: August 15, 2026**
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'pwa-install', 'Install as an App (PWA)', 'Fewer is a progressive web app: install it to your home screen for an app-like experience with offline support, splash screen, and standalone window.', 'Fewer is a progressive web app. Once you open [app.fewer.directory](https://app.fewer.directory/app) in a supported browser, you can install it like a native app.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'settings', 'Settings, Power User Mode & Notifications', 'Configure Fewer via the Settings dialog: theme, minimap, node dimensions, power user mode, notifications, and the About/Help tabs.', 'The gear icon in the top navbar opens **Settings**, a dialog with four tabs: **About**, **Appearance**, **Advanced**, and **Help**.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'sharing', 'Sharing Graphs', 'Generate shareable links for your graph. Small graphs embed in the URL; large graphs use a short server-backed link. Saved graphs can be shared publicly or invite-only.', 'Fewer lets you share graphs with anyone via a link. Two ways to share:
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'shortcuts', 'Keyboard Shortcuts', 'Complete reference for all keyboard shortcuts in Fewer. Navigation, editing, search, export, and view controls.', 'Fewer is primarily a keyboard driven application. Most of the common operations can be triggered using keyboard shortcuts. Here is a comprehensive list of all available shortcuts:
...
- **Ctrl+click** a node handle removes its connected connections', NULL, NULL, NULL, true)
on conflict (type, slug) do update
  set title = excluded.title,
      description = excluded.description,
      content = excluded.content,
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'terms', 'Terms of Use', 'The terms and conditions governing your use of the fewer web application and related services, written to be fair, clear, and easy to understand.', '**Last updated: August 15, 2026**
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'theming', 'Theming', 'Customize Fewer''s appearance with built-in light/dark modes, 18 theme presets, and a custom color editor with per-color opacity.', 'Fewer has been designed with a dual color theme in mind. It comes with a light and dark theme, both of which are customizable, plus 18 hand-tuned presets and a full custom theme editor.
...
insert into public.content_pages
  (type, slug, title, description, content, author, date, tags, published)
values ('docs', 'watch', 'Watch File Indexes', 'Watch public file indexes and get a daily email digest when they change. Toggle watching on import and manage watched indexes in Settings.', 'Fewer can watch the public file indexes you import and email you a daily digest when they change. This is useful for keeping an eye on a directory that updates over time, such as a data mirror or a downloadable-assets folder.
