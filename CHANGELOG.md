...
- **Unified import shortcut + keyboard-friendly dialog**: importing now uses a single **Alt+I** shortcut that opens the unified import flow (folder / file / URL / cloud are all picked inside it). The old `Alt+U` and `Alt+L` shortcuts (which dispatched events with no listener after the flow rewrite) and their docs/dialog entries were removed. The import dialog's origin picker is now fully keyboard-accessible: it's a roving-tabindex `radiogroup` with **arrow-key** navigation between sources, auto-focuses the selected source on open, and shows an inline keyboard hint.
