...
- **Unified import shortcut + keyboard-friendly dialog**: importing now uses a single **Alt+I** shortcut that opens the unified import flow (folder / file / URL / cloud are all picked inside it). The old `Alt+U` and `Alt+L` shortcuts (which dispatched events with no listener after the flow rewrite) and their docs/dialog entries were removed. The import dialog is now keyboard-driven throughout step 1: the origin picker is a roving-tabindex `radiogroup` with **arrow-key** navigation (auto-focuses on open, with an inline hint); pressing **Enter** on an origin advances to the next control — **folder** jumps to step 2, **file** focuses the format tiles, **URL** focuses the address field, **cloud** focuses the linked-account list. The file format tiles and cloud account list also support arrow-key navigation.
- **Responsive default layout direction**: on screens smaller than full HD (1920×1080) the initial layout direction is now LR (wide layout fits better on small/portrait displays); on full HD and larger it stays TB. SSR/headless still defaults to TB, saved graphs and the sidebar control can override.
