...
- Account profile panel: Settings → Account (formerly About) now lets signed-in users save a first name, last name, and username — stored on their account via the auth user's metadata — and keeps the Sign out and Delete account actions in the same tab.
