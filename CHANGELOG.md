...
- **Cloud connections (OAuth account linking)**: signed-in users can link cloud accounts (GitHub initial; Google Drive, OneDrive, SharePoint, Azure DevOps, Azure Blob planned) and browse/import their folders into the graph. New Cloud sidebar section (link/unlink/browse), Cloud Browser dialog with lazy folder navigation + depth-limited import, and "Open in provider" context-menu action on imported nodes that opens the folder/file in its provider in a new tab. Server-side OAuth: `/api/cloud/connect` (redirect to provider consent), `/api/cloud/callback` (exchange code, encrypt+store tokens), `/api/cloud/connections` (list/unlink), `/api/cloud/list` (lazy folder listing), `/api/cloud/tree` (build subtree). Tokens encrypted (AES-256-GCM) via `CONNECTIONS_ENCRYPTION_KEY`, owner-only RLS. Migration `0011_cloud_connections.sql`. Provider adapters in `src/lib/fewer/cloud/providers/`. Env: CONNECTIONS_ENCRYPTION_KEY, GITHUB_CLIENT_ID/SECRET, GOOGLE_CLIENT_ID/SECRET, MICROSOFT_CLIENT_ID/SECRET/TENANT, AZURE_BLOB_STORAGE_ACCOUNT. Docs: `/docs/cloud` + OAuth setup in `/docs/deployment`.
