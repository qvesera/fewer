...
- **Edge delete could re-select a previously-selected node** (e.g. "A copy"): React Flow-driven clicks don't update the store's per-node `selected` flags, so the stale flag resurrected the selection whenever the canvas rebuilt nodes on a graph edit (like deleting an edge). The node rebuild now derives selection from the canonical `selectedNodeIds` — which is always cleared when another edge/node is picked.
