...
- **Privacy Policy & Terms of Use**: new legal pages at `/docs/privacy` and `/docs/terms` (rendered from `content/docs/privacy.md` and `content/docs/terms.md`), a "Legal" section in the docs sidebar, and a site footer with Privacy/Terms links added to `DocsLayout`. Both pages include a "not legal advice" disclaimer rendered as a styled blockquote, which required adding blockquote support to the custom `MarkdownRenderer`.
