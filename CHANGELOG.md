...
- Account profile panel: Settings → Account (formerly About) now lets signed-in users save a first name, last name, and username — stored per account in the new owner-only `profiles` DB table (migration `0019_profiles.sql`, `/api/profile`) — and keeps the Sign out and Delete account actions in the same tab.
