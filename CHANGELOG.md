...
- Two module import cycles are broken: Tag now lives in types.ts (re-exported from tags.ts), and the panel-area primitives (AreaEditor, PanelArea, createArea, generateAreaId and their width defaults) moved into panelTree.ts with panelLayout.ts re-exporting them — so types.ts no longer imports tags.ts and panelTree no longer imports panelLayout, while every existing import site keeps working unchanged. The server-side GitHub tree assembly is now a documented buildTreeEntries helper, the legacy localStorage-to-sessionStorage graph lift is a named liftLegacyGraphIntoSessionStorage, and view-settings sanitising shares one stringIds plus sanitizeSubtrees helper instead of five inline filters. Behavior unchanged.
...
- Color luminance and hex parsing are now one shared contract in themeColors.ts: the exported luma / lumaHex / isLightRgb replace six copies of the BT.601 formula (two in themeColors, three in themeSlice, one in use-dark-background) and nine hand-rolled hex-to-rgb parses, and parseLegacyColor no longer re-runs the hex regex before calling hexToRgb. The new form is exact at integer greys, where the old 0.299 form drifted one ULP (#808080 read as 127.99999999999999), so the 128 surface midpoint and the 140 primary-foreground cutoff resolve identically for every color. computePrimaryFgFinal drops its dead foreground-contrast scaffolding (a four-branch ternary that always returned white) plus the fg and isLight arguments only that dead code read. Behavior unchanged.
- The custom-theme to shadcn/ui variable map is now a pure function in themeColors.ts: deriveShadcnVars, computePrimaryFgFinal and the PRIMARY_FG_LUMA_THRESHOLD constant moved out of themeSlice.ts, which now holds only the DOM writes, and the map is covered by tests pinning the dark/light card and muted offsets, the border tints, the accent opacity passthrough and the 140 primary-foreground cutoff. Behavior unchanged: the pre-move and post-move implementations were diffed over every integer grey, invalid inputs, opacity extremes and 5,000 random themes with no differing variable.
- Both show-subtree actions now share one reveal walk (walkSubtreeReveal in graphSlice.ts): showSubtree's hand-rolled breadth-first copy of collectShowSubtrees is gone, and the single difference between them -- the singular action always seeds the card the user clicked, so an already-visible folder still reveals hidden children beneath it -- is now an explicit seeding rule at the call site instead of a second traversal of the same edges. Behavior unchanged: both paths were compared against the removed implementations over 500 random graphs each (orphan nodes, hidden roots, overlapping hidden/independently-hidden sets) with no differing reveal set, and the singular path gains coverage for the visible-root and visible-descendant walk cases.
- Every parent-id index is now built by one helper: parentMapOf(edges) in src/lib/fewer/validation.ts, the inverse of the existing childrenMapOf. Seven copies of the same two-line child-id-to-parent-id loop are gone (two in hiddenGroups, one in the reveal helpers, four in the graph slice), and the last-edge-wins tie-break for an imported fan-in is now documented and pinned by tests. Behavior unchanged.
- The Settings dialog's decisions moved into one tested module (src/lib/fewer/settingsModel.ts): profile responses are normalised defensively (unknown JSON cannot corrupt the account tab), the unsaved-changes check and save body share one normalisation rule, the saved-graphs/watched-indexes meters share one meter calculation instead of two hand-rolled copies, the tab strip is derived from a visibleTabs() rule (documenting why the Advanced tab is desktop-only unless advanced mode is on), and the edge-stroke/theme option lists and minimap bounds follow. Behavior unchanged; the dialog renders the same tabs and meters in the same order.
- Pure display helpers (handle positions, file icons, size formatting, provider labels, rename selection, folder child counts) moved out of CustomNode into a tested nodeDisplay module
...
- Crown Shyness intensity and sibling sort order are applied again on per-view canvases: a leaf that re-derives its own layout (view-specific hiding, collapsed folders, or a direction override) no longer falls back to the engine's built-in 1x spacing, so the Settings sliders are honoured everywhere
- Organize actually re-flows the tree again: any per-view settings entry made it clear that view's card positions and stop without re-running the layout, and because clearing positions leaves the key behind every session accumulates one — so clicking Organize (sidebar, Layout panel, canvas context menu, Alt+R) looked like a no-op and a new Crown Shyness intensity rendered identically at 0 and 3. Organize now always hands the view to the layout engine (shared layout when no view is active), and the canvas and Organize share one derivation predicate so a view that derives its own layout (view-specific hiding, collapsed folders, direction override) re-derives instead of being half-organised
