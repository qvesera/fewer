...
- Settings gains a dedicated Account tab (separate from About): signed-in users can save a first name, last name, and username — stored per account in the new owner-only `profiles` DB table (migration `0019_profiles.sql`, `/api/profile`) — and it keeps the Sign out and Delete account actions in the same tab.
