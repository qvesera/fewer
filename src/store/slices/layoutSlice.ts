...
import type { LayoutDirection, EdgeStyle, EdgeStrokeStyle, FewerEdge } from "@/lib/fewer/types";
...
/**
 * Monotonic edge-id sequence for setDirection: ids are rewritten so React
 * Flow fully remounts edge components with the new handle orientation. A
 * counter keeps ids unique and deterministic — the previous
 * Date.now()+Math.random scheme could collide within one millisecond and
 * made the ids unassertable in tests. All edges are remapped in one write,
 * so no stale id can survive alongside a fresh one.
 */
let edgeIdSeq = 0;
const nextEdgeId = (source: string, target: string) => `e-${source}-${target}-${++edgeIdSeq}`;

/** Apply the base stroke style's dash to every edge, optionally setting the
 * animated motion flag. strokeDasharray is always written (undefined for
 * solid) so a previous pattern never survives a style switch. */
function withDashAndMotion(edges: FewerEdge[], strokeStyle: EdgeStrokeStyle, animated?: boolean): FewerEdge[] {
  return edges.map((e) => {
    const next: FewerEdge = { ...e, style: { ...e.style, strokeDasharray: edgeDashPattern(strokeStyle) } };
    if (animated !== undefined) next.animated = animated;
    return next;
  });
}
...
export const createLayoutSlice: LayoutSliceCreator = (set, get) => ({
  ...
  setEdgeStyle: (style) => {
    set((s) => ({
      edgeStyle: style,
      edges: s.edges.map((e) => ({ ...e, type: edgeTypeFromStyle(style) })),
  ...
  setEdgeAnimated: (animated) => {
    set((s) => ({
      ...
      edgeAnimated: animated,
      edges: withDashAndMotion(s.edges, s.edgeStrokeStyle, animated),
      graphVersion: s.graphVersion + 1,
    }));
  },
  ...
  setEdgeStrokeStyle: (strokeStyle) => {
    set((s) => ({
      ...
      edges: withDashAndMotion(s.edges, strokeStyle),
      graphVersion: s.graphVersion + 1,
    }));
  },
  ...
  setEdgeWidth: (width) => {
    ...
    set((s) => ({
      edgeWidth: clamped,
      edges: s.edges.map((e) => ({ ...e, style: { ...e.style, strokeWidth: clamped } })),
      graphVersion: s.graphVersion + 1,
    }));
  },
  ...
  setCornerRadius: (radius) => {
    ...
    set((s) => ({
      ...
      edges: s.edges.map((e) => ({ ...e, pathOptions: { borderRadius: clamped } })),
      graphVersion: s.graphVersion + 1,
    }));
  },
