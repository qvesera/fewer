...
const STEPS: TutorialStep[] = [
  {
    ...
    targetSelector: null,
    actionType: "none",
    tips: ["Click 'Next' to begin the walkthrough", "You can skip anytime — the tutorial won't show again"],
  },
  {
    icon: Plus,
    ...
    targetSelector: 'button:has(> span:contains("Sample")), button[title*="Sample"]',
    actionType: "click",
    actionHint: "Click the 'Sample' button in the toolbar",
    watchState: null,
    tips: ["This loads a pre-built project tree so you can explore immediately"],
  },
  {
    icon: Layers,
  ...
  },
  {
    ...
    tips: ["Search matches filenames, paths, and extensions", "Click any result to zoom to that node"],
  },
  {
    icon: Keyboard,
    ...
    tips: ["F2 renames · Enter opens files · Delete removes · H hides · Shift+H unhides"],
  },
  {
    icon: Palette,
  ...
  },
  {
    ...
    tips: ["Toggle 'Export selected only' to export just a subtree", "JSON exports can be re-imported via 'Import from File'"],
  },
  {
    icon: Keyboard,
...
export function TutorialDialog() {
  ...
  useEffect(() => {
    const checkWelcome = () => {
      const welcomeDialog = document.querySelector("[data-testid='welcome-dialog']") ||
        document.querySelector('[role="dialog"]:has(> div > div > h2:contains("Graphir Pro Max Ultra"))');
      ...
      const welcomeButtons = Array.from(document.querySelectorAll('button')).filter(
        (b) => b.textContent?.includes("Load sample") || b.textContent?.includes("Open my directory") || b.textContent?.includes("Start exploring")
      );
  ...
  useEffect(() => {
    ...
    if (!current.watchState) {
      Promise.resolve().then(() => setActionCompleted(current.actionType === "none"));
      return;
    }
  ...
  return (
    <>
      {/* Spotlight overlay */}
      <SpotlightOverlay targetSelector={current.targetSelector} onDismiss={() => {}} />
            ...
            {STEPS.map((_, i) => (
              <div
                ...
                className={cn(
                  ...
                  i === step ? "w-6 bg-orange-500" : i < step ? "w-1.5 bg-orange-400/50" : "w-1.5 bg-muted-foreground/30"
                )}
              />
            ))}
          </div>
