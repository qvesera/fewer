...
export function ImportDialog({
  ...
  const basicOptions = (
    <div className="space-y-2">
      ...
      <p className="text-[10px] text-muted-foreground">
        How many levels of subdirectories to scan. 0 = unlimited, 1 = top
        level only.
      </p>
    </div>
  );

  // Advanced options (hidden files, vendored dirs, etc.)
      ...
      <div className="flex items-center justify-between rounded-lg border border-border/40 p-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              Files and folders starting with a dot (e.g. .gitignore,
              .env)
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between rounded-lg border border-border/40 p-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              When off, only the directory structure is imported (no file
              nodes)
            </p>
          </div>
        </div>
  ...
  const summary = (
          ...
          <span>{options.includeHidden ? "Included" : "Excluded"}</span>
            ...
            <span>
              {options.includeFiles ? "Imported as nodes" : "Folders only"}
            </span>
          </div><div className="flex justify-between">
            <span className="text-muted-foreground">Extension filter</span>
            <span>
              {options.extensions.length > 0
                ? `${options.extensions.length} exts`
                : "All files"}
            </span>
          </div></>
            )}
